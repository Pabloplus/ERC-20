...
/* taking ideas from Nebulas ERC-20 Token. 이 아이디어는 네뷸라스 ERC-Token에서 차용하였습니다/ */
contract SafeMath {
...
/*  ERC 20 token ERC-20 토큰 컨트랙트 중 스탠다드 토큰 부분입니다. */
...
contract PabloPlusToken is StandardToken, SafeMath {

    // metadata
    string  public constant name = "PabloPlus2 Token";
    string  public constant symbol = "PAB+2";
    uint256 public constant decimals = 18;
    string  public version = "1.01";

    // contracts
    address public ethFundDeposit;          // deposit address for ETH for PabloPlus Team.
    address public newContractAddr;         // the new contract for PabloPlus token updates;
    ...
    uint256 public tokenExchangeRate = 15000000;             // 15000000 PAB+ tokens per 1 ETH. 1ETH당 1500만 PAB+
    ...
    /// @dev sends ETH to PabloPlus team
    function transferETH() isOwner external {
        if (this.balance == 0) throw;
        if (!ethFundDeposit.send(this.balance)) throw;
    }

    /// @dev allocates PAB+2 tokens to pre-sell address.
